...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        # print('Number of parameters: %.2fM' % (total / 1))
        return model
    ...
    def vali(self, vali_data, vali_loader, criterion,is_test = True):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.model == 'TFPMixer' and is_test == False:
    ...
    def test(self, setting, test=0):
        ...
        #cp_forecast = self.model.conformal_predict(
        ...
        #cp_forecast
        mae, mse, rmse, mape, mspe,pinballhigh,pinballlow = metric(preds, trues)

        #wandb.log({"test mae": mae," test mse":mse})
        print('mse:{}, mae:{},pinballhigh:{}, pinballlow:{}'.format(mse, mae,pinballhigh,pinballlow))

        #tau = 0.1
        #loss = np.where(trues >= preds, tau * (trues - preds), (1 - tau) * (preds - trues))
        #avg_loss = np.mean(loss)
        #print(f'Pinball Loss0.1: {avg_loss}')
        #tau = 0.9
        #loss = np.where(trues >= preds, tau * (trues - preds), (1 - tau) * (preds - trues))
        #avg_loss = np.mean(loss)
        #print(f'Pinball Loss0.9: {avg_loss}')
        f = open("result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{},pinballhigh:{}, pinballlow:{}'.format(mse, mae,pinballhigh,pinballlow))
        f.write('\n')
        f.write('\n')
        f.close()
